  ...
  .my_find_qtl <- function( MMt_sqrt_and_sqrtinv, Zmat=NULL, geno, availmemGb,  selected_loci, MMt, invMMt, best_ve, best_vg,
 ...
 start <- Sys.time()
    H <- Eagle:::calculateH(MMt=MMt, varE=best_ve, varG=best_vg, Zmat=Zmat )
 end <- Sys.time()
#           print(c(" H =  ", end-start))


    if(!quiet)
      Eagle:::doquiet(dat=H, num_markers=5, lab="H")

 start <- Sys.time()
    P <- Eagle:::calculateP(H=H, X=currentX  )
 end <- Sys.time()
# print(c(" P =  ", end-start))

    if(!quiet)
      Eagle:::doquiet(dat=P, num_markers=5 , lab="P")
...
# print(c(" MMt_sqrt_and_sqrtinv  =  ", end-start))
    ...
    if(!quiet){
      Eagle:::doquiet(dat=MMt_sqrt_and_sqrtinv[["sqrt_MMt"]], num_markers=5, lab="sqrt(M %*% M^t)")
      Eagle:::doquiet(dat=MMt_sqrt_and_sqrtinv[["inverse_sqrt_MMt"]], num_markers=5, lab="sqrt(M %*% M^t)^-1")
    }
    if(!quiet ){
 ...
 start <- Sys.time()
       hat_a <- Eagle:::calculate_reduced_a(Zmat=Zmat, varG=best_vg, P=P,
 ...
 #print(c(" hat_a  =  ", end-start))
    ...
    if(!quiet)
      Eagle:::doquiet(dat=hat_a, num_markers=5, lab="BLUPs")
 ...
 start <- Sys.time()
    var_hat_a    <- Eagle:::calculate_reduced_vara(Zmat=Zmat, X=currentX, varE=best_ve, varG=best_vg, 
                       invMMt=invMMt,
                       MMtsqrt=MMt_sqrt_and_sqrtinv[["sqrt_MMt"]],
                       quiet = quiet )
 ...
 #print(c(" var_hat_a  =  ", end-start))
    ...
    if(!quiet)
      Eagle:::doquiet(dat=var_hat_a, num_markers=5, lab="SE of BLUPs")
    ...
    if(!quiet ){
     ...
     a_and_vara  <- Eagle:::calculate_a_and_vara(geno = geno,
     ...
     if(!quiet){
       Eagle:::doquiet(dat=a_and_vara[["a"]], num_markers=5, lab="BLUPs for full model")
       Eagle:::doquiet(dat=a_and_vara[["vara"]], num_markers=5, lab="SE of BLUPs for full model")
     }
    ...
    if(!quiet)
      Eagle:::doquiet(dat=tsq, num_markers=5, lab="outlier test statistic")
    ...
    res[["outlierstat"]] <- tsq
    res[["hat_a"]] <- a_and_vara[["a"]]
    res[["var_hat_a"]] <- a_and_vara[["vara"]]
    res[["hat_a_reduce"]] <- hat_a
    res[["var_hat_a_reduce"]] <- var_hat_a
