...
```{r}
PWD <- paste0(getwd(),"/Test_Result03/")
# PWD <- "/work_home/jwang/stage_partie2/Test_Result03/"
...
A_ <- c(1,5,10,40,60,100,140,180,220,260,300,350,400,500,600,800,1000)

args_X <- list()

for (a_idx in 1:length(A_)) {
  ...
  for ( itr in 1:simulation_times) {
    ...
    args_X[[simulation_times*(a_idx-1) + itr]] <- list("adS" = adS, "afS" = afS, "s_label" = s_label, 
                          "gamma" = gamma, "itr" = itr, 
...
for ( a_idx in 1:length(A_)) {
  ...
  Eagle_S_selected_locus_G1_history[[a_idx]] <- list()
  Eagle_S_selected_locus_G2_history[[a_idx]] <- list()
  EMG_S_selected_locus_history[[a_idx]] <- list()
  eagle_S_excute_time_history[[a_idx]] <- list()
  emg_S_excute_time_history[[a_idx]] <- list()
  ...
  for ( itr in 1:simulation_times) {
    ...
    Eagle_S_selected_locus_G1_history[[a_idx]][[itr]] <- eagle_res$Eagle_selected_locus_G1 
    Eagle_S_selected_locus_G2_history[[a_idx]][[itr]] <- eagle_res$Eagle_selected_locus_G2 
    EMG_S_selected_locus_history[[a_idx]][[itr]] <- emg_res$EMG_selected_locus 
    eagle_S_excute_time_history[[a_idx]][[itr]] <- eagle_res$eagle_excute_time
    emg_S_excute_time_history[[a_idx]][[itr]] <- emg_res$emg_excute_time 
  }
}
...
for ( a_idx in 1:length(A_) ) {
  ...
  for (itr in 1:simulation_times) {
    eagle_qtl_find1 = length(Eagle_S_selected_locus_G1_history[[a_idx]][[itr]])
    eagle_qtl_find2 = length(Eagle_S_selected_locus_G2_history[[a_idx]][[itr]])
    eagle_qtl_find = eagle_qtl_find1 + eagle_qtl_find2
    
    eagle_TP1 <- length(intersect(Sf_neighborhood5, Eagle_S_selected_locus_G1_history[[a_idx]][[itr]]))
    eagle_TP2 <- length(intersect(Sd_neighborhood5, Eagle_S_selected_locus_G2_history[[a_idx]][[itr]]))
    ...
    emg_qtl_find1 = length(EMG_S_selected_locus_history[[a_idx]][[itr]]$G1)
    emg_qtl_find2 = length(EMG_S_selected_locus_history[[a_idx]][[itr]]$G2)
    emg_qtl_find = emg_qtl_find1 + emg_qtl_find2
    
    emg_TP1 <- length(intersect(Sf_neighborhood5, EMG_S_selected_locus_history[[a_idx]][[itr]]$G1))
    emg_TP2 <- length(intersect(Sd_neighborhood5, EMG_S_selected_locus_history[[a_idx]][[itr]]$G2))
  ...
  eagle_TP_list[[a_idx]] <- eagle_TP
  eagle_FP_list[[a_idx]] <- eagle_FP
  eagle_FN_list[[a_idx]] <- eagle_FN

  emg_TP_list[[a_idx]] <- emg_TP
  emg_FP_list[[a_idx]] <- emg_FP
  emg_FN_list[[a_idx]] <- emg_FN
}
```

#### TP, FP, FN en fonction des effets additives

```{r}
library(ggplot2)

a = rep(paste0(A_), each=simulation_times)
algo = rep(c("eagle","emg"), each=simulation_times)
TP = c()

for ( a_idx in 1:length(A_) ) {
  TP = c(TP, eagle_TP_list[[a_idx]])
  TP = c(TP, emg_TP_list[[a_idx]])
}
...
FP= c()
for ( a_idx in 1:length(A_) ) {
  FP = c(FP, eagle_FP_list[[a_idx]])
  FP = c(FP, emg_FP_list[[a_idx]])
}
...
FN= c()
for ( a_idx in 1:length(A_) ) {
  FN = c(FN, eagle_FN_list[[a_idx]])
  FN = c(FN, emg_FN_list[[a_idx]])
}
...
png(filename="FN.png", width=1200, height=600)

ggplot(data_FN, aes(x=reorder(a, as.numeric(a)), y=FN, fill=algo), main = "False Positive") + geom_boxplot() + geom_point(data = FN_mean_eagle, mapping = aes(x = reorder(a, as.numeric(a)), y = average), color="red") + geom_line(data=FN_mean_eagle, mapping = aes(x = reorder(a, as.numeric(a)), y = average, group=1), colour = "red") + geom_point(data = FN_mean_emg, mapping = aes(x = reorder(a, as.numeric(a)), y = average), color="blue") + geom_line(data=FN_mean_emg, mapping = aes(x = reorder(a, as.numeric(a)), y = average, group=1), colour = "blue")
...
for (trancate_idx in 1:max_trancate_num) {
    ...
    for (simulation_num in 1:total_test_num) {
      ...
      trancated_eagle_res <- trancate_eagle_result(G1 = res_eagle$Eagle_selected_locus_G1,
                                                   G2 = res_eagle$Eagle_selected_locus_G2,
...
total_total_selected_snps_num = rep(paste0(1:max_trancate_num), each = total_test_num*2)
