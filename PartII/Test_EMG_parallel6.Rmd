...
test_bs <- rep(bs, simulation_times*length(hs))
test_tn <- rep(floor(seq(1,simulation_times+0.5,0.5)), length(hs))
...
PWD <- paste0(getwd(),"/Test_Result06/")
# PWD <- paste0(getwd(),"/Test_Result07/")

label <- "[ad_=%d][af_=%d][gamma=%.2f][h=%.2f][Poly.size.d=%d][Poly.size.f=%d][Poly.effet.d=%.3f][Poly.effet.f=%.3f][tn=%d][b=%.2f]"
...
sl_label <- function(h, b, tn) {
  return(sprintf(label, ad_, af_, gamma, h, Poly.size.d, Poly.size.f, Poly.effet.d, Poly.effet.f, tn, b))
}
  ...
  while(TRUE) {
    ...
    if (a_ratio < (b+0.02) && a_ratio > (b-0.02) && !file.exists(model_fname(h,b,tn))) {
      ...
      save_model(Sd = Sd, Sf = Sf, adS = adS, afS = afS, 
                 ad_S = adS, af_S = af_S, E = E, fname = model_fname(h,b,tn))
...
simu <- function (a_) {
  ...
  vh <- var(Uh)
  va <- var(Ua)
  if (va/vh < b + 0.05) return(a_)
}

A_ <- c()
for (i in 1:10) {
  A_ <- unique(c(A_,unlist(mclapply(1:100, simu, mc.cores = getOption("mc.cores", 48L)))))
}
A_ # so 10 is OK
```
...
```{r}
mcmapply(sl, h = test_hs, b = test_hs, tn = test_tn, 
  ...
  if (DO_EMG) {
    ...
    # if (!file.exists(tfname)) {
      ...
      EMG_selected_locus <- EMG(Y = Y, G1 = Mf_technow*2, G2 = Md_technow*2, Z1 = Zf, Z2 = Zd, 
                  genome_names = c("Flint", "Dent"), gamma = gamma, max_itr_num = max_itr_num, Method = "Reml")
...
mcmapply(test_case, h = rep(hs, each = simulation_times*length(bs)), 
         b = rep(bs, simulation_times*length(hs)), 
         tn = rep(floor(seq(1,simulation_times+0.5,0.5)), length(hs)), 
...
args_X <- data.frame("h" = rep(hs, each = simulation_times*length(bs)), 
                     "b" = rep(bs, simulation_times*length(hs)), 
                    "tn" = rep(floor(seq(1,simulation_times+0.5,0.5)), length(hs)))
...
if (DO_EMG) {
  for (tn in c(1:emg_total_test_times)) {
    ...
    # res_emg_list[[tn]] <- readRDS(file = paste0(PWD, "EMG_Results/emg_resultat_", sl_label(h,b,tn)), "[K=Eagle]")
  }
}
...
lines(1:max_trancate_num, unlist(lapply(data.frame(emg_TP1), max)), col = "blue")
lines(1:max_trancate_num, unlist(lapply(data.frame(emg_TP2), max)), col = "yellow")
lines(1:max_trancate_num, unlist(lapply(data.frame(eagle_TP), max)), col = "black")
legend("topleft", legend=c("emg total", "flint", "dent", "eagle total"), col=c("red", "blue", "yellow", "black"), lty = 1:2, cex=0.8)

plot(1:max_trancate_num, unlist(lapply(data.frame(eagle_TP), max)),type = "l", ylim = c(0,nqtl.d +nqtl.f), col = "red",
...
lines(1:max_trancate_num, unlist(lapply(data.frame(eagle_TP1), max)), col = "blue")
lines(1:max_trancate_num, unlist(lapply(data.frame(eagle_TP2), max)), col = "yellow")
lines(1:max_trancate_num, unlist(lapply(data.frame(emg_TP), max)), col = "black")
legend("topleft", legend=c("eagle total", "flint", "dent", "emg total"), col=c("red", "blue", "yellow", "black"), lty = 1:2, cex=0.8)
...
lines(1:max_trancate_num, unlist(lapply(data.frame(emg_TP1), mean)), col = "blue")
lines(1:max_trancate_num, unlist(lapply(data.frame(emg_TP2), mean)), col = "yellow")
lines(1:max_trancate_num, unlist(lapply(data.frame(eagle_TP), mean)), col = "black")
legend("topleft", legend=c("emg total", "flint", "dent", "eagle total"), col=c("red", "blue", "yellow", "black"), lty = 1:2, cex=0.8)

plot(1:max_trancate_num, unlist(lapply(data.frame(eagle_TP), mean)),type = "l", ylim = c(0,nqtl.d +nqtl.f), col = "red",
...
lines(1:max_trancate_num, unlist(lapply(data.frame(eagle_TP1), mean)), col = "blue")
lines(1:max_trancate_num, unlist(lapply(data.frame(eagle_TP2), mean)), col = "yellow")
lines(1:max_trancate_num, unlist(lapply(data.frame(emg_TP), mean)), col = "black")
legend("topleft", legend=c("eagle total", "flint", "dent", "emg total"), col=c("red", "blue", "yellow", "black"), lty = 1:2, cex=0.8)
...
plot(FPR_EMG,TPR_EMG, type = "l", xlim=c(0,0.0025), ylim=c(0,1), xlab = "FPR (False Positive Rate)",ylab = "TPR (True Positive Rate)",
  main = sprintf("EMG ROC Curve heritability=%.2f balance=%.2f window size=%d",h,b,window_size), col = "red")
lines(FPR_EMG1,TPR_EMG1, col="blue")
lines(FPR_EMG2,TPR_EMG2, col="yellow")
lines(FPR_EAG,TPR_EAG, col="black")

plot(FPR_EAG,TPR_EAG, type = "l", xlim=c(0,0.0025), ylim=c(0,1), xlab = "FPR (False Positive Rate)",ylab = "TPR (True Positive Rate)",
  main = sprintf("EAGLE ROC Curve heritability=%.2f balance=%.2f window size=%d",h,b,window_size), col = "red")
lines(FPR_EAG1,TPR_EAG1, col="blue")
lines(FPR_EAG2,TPR_EAG2, col="yellow")
lines(FPR_EMG,TPR_EMG, col="black")
...
for (tn in c(1:simulation_times)) {
  ...
  if (method == "Reml") {
    ...
    # ML
    res_emg_list[[tn]] <- readRDS(file = paste0(PWD, "EMG_Results/emg_resultat_", sl_label(h,b,tn))) 
    print(paste0(PWD, "EMG_Results/emg_resultat_", sl_label(h,b,tn)))
  }
}
...
for (trancate_idx in 1:max_trancate_num) {
    ...
    # for (tn in c(1:eagle_total_test_times)) {
    #   
    #   # res_eagle <- readRDS(file = paste0(PWD, "Eagle_Results/eagle_resultat_", sl_label(h,b,tn)))
    #   # print(paste0(PWD, "Eagle_Results/eagle_resultat_", sl_label(h, b, tn)))
    #   # mdl <- readRDS(model_fname(h,b,tn))
    #   
    #   res_eagle <- res_eag_list[[tn]]
    #   mdl <- model_list[[tn]]
    #   
    #   Sd  <- mdl$Sd
    #   Sf  <- mdl$Sf
    #   
    #   Sd_neighborhood <- nhbrhd(Sd, neighborhood_radius)
    #   Sf_neighborhood <- nhbrhd(Sf, neighborhood_radius)
    #   
    ...
    for (tn in c(1:emg_total_test_times)) {
      ...
      # res_emg <- readRDS(file = tfname) 
      # mdl <- readRDS(model_fname(h,b,tn))
  
      res_emg <- res_emg_list[[tn]]
      mdl <- model_list[[tn]]
...
plot(1:max_trancate_num, unlist(lapply(data.frame(emg_TP), max)),type = "l",
     xlab = "selected SNPs number", ylab = "True Positive", main = sprintf("EMG MAX heritability=%.2f balance=%.2f neighborhood=%d",h,b,neighborhood_radius))

plot(1:max_trancate_num, unlist(lapply(data.frame(eagle_TP), max)),type = "l",
     xlab = "selected SNPs number", ylab = "True Positive", main = sprintf("EAGLE MAX heritability=%.2f balance=%.2f neighborhood=%d",h,b,neighborhood_radius))

plot(1:max_trancate_num, unlist(lapply(data.frame(emg_TP), mean)),type = "l",
     xlab = "selected SNPs number", ylab = "True Positive", main = sprintf("EMG MEAN heritability=%.2f balance=%.2f neighborhood=%d",h,b,neighborhood_radius))

plot(1:max_trancate_num, unlist(lapply(data.frame(eagle_TP), mean)),type = "l",
     xlab = "selected SNPs number", ylab = "True Positive", main = sprintf("EAGLE MEAN heritability=%.2f balance=%.2f neighborhood=%d",h,b,neighborhood_radius))
...
# TPR = TP / P
TPR_EMG <- unlist(lapply(data.frame(emg_TP), mean)) / 20

# FPR = FP / N
FPR_EMG <- unlist(lapply(data.frame(emg_FP), mean)) / (ncol(Mf)+ncol(Md)-20*(neighborhood_radius*2+1))
  
plot(FPR_EMG,TPR_EMG, type = "l", xlim=c(0,0.001), ylim=c(0,1), xlab = "FPR False Positive Rate",ylab = "TPR True Positive Rate",
  main = sprintf("EMG ROC Curve heritability=%.2f balance=%.2f neighborhood=%d",h,b,neighborhood_radius))

points(FPR_EMG,TPR_EMG, pch = 16)

# TPR = TP / P
TPR_EAG <- unlist(lapply(data.frame(eagle_TP), mean)) / 20

# FPR = FP / N
FPR_EAG <- unlist(lapply(data.frame(eagle_FP), mean)) / (ncol(Mf)+ncol(Md)-20*(neighborhood_radius*2+1))

plot(FPR_EAG,TPR_EAG, type = "l", xlim=c(0,0.001), ylim=c(0,1), xlab = "FPR",ylab = "TPR",
  main = sprintf("EAGLE ROC Curve heritability=%.2f balance=%.2f neighborhood=%d",h,b,neighborhood_radius))

points(FPR_EAG,TPR_EAG, pch = 16)
